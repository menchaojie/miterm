...
function filterToValue(filter: CategoryFilter): string {
...
export function HostListPage({
  ...
  const selectedCategory =
    typeof selectedFilter === "number"
      ? categories.find((c) => c.id === selectedFilter)
      : undefined;
  ...
  return (
    <div className="host-list-page">
      ...
      <div className="host-filter-bar">
        <label className="host-filter-label">
          分类筛选
          <select
            className="host-filter-select"
            disabled={busy}
            value={filterToValue(selectedFilter)}
            onChange={(e) => onSelectFilter(parseFilterValue(e.target.value))}
          >
            <option value="all">全部（{hosts.length}）</option>
            <option value="uncategorized">
              未分类（{hosts.filter((h) => h.categoryId == null).length}）
            </option>
            {categories.map((cat) => (
              <option key={cat.id} value={cat.id}>
                {`${cat.name}（${hosts.filter((h) => h.categoryId === cat.id).length}）`}
              </option>
            ))}
          </select>
        </label>
        <div className="host-filter-actions">
          <button
            type="button"
            className="btn-secondary"
            disabled={busy}
          ...
          <button
            ...
            title={
              selectedCategory ? "重命名当前分类" : "请先筛选到某个分类"
            }
            onClick={() =>
              selectedCategory && onRenameCategory(selectedCategory)
            }
          >
            重命名分类
          </button>
          <button
            ...
            disabled={busy || !selectedCategory}
            title={
              selectedCategory ? "删除当前分类" : "请先筛选到某个分类"
            }
            onClick={() =>
              selectedCategory && onDeleteCategory(selectedCategory)
            }
          >
            删除分类
          </button>
