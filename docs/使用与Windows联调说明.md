...
| 远程文件 | SSH 会话侧栏「远程文件」打开 SFTP；默认跟随终端目录；单文件/整夹上传与下载 |
| 工作区 | 侧栏「工作区」保存/打开命名组合（主机夹分屏或并发）；含布局与 cwd；打开时新开一级 Tab |
...
│   │   ├── WorkspaceSidebar.tsx # 侧栏：工作区 + 远程文件
...
| Tab | 可否关闭 | 内容 |
|-----|----------|------|
| 主机列表（最左，固定） | 否 | 分类筛选 + 主机表（含本地终端条目）；可勾选多台 |
| 并发 · N 台 | 是（=断开组内全部） | 同屏网格；浮层「并/单」；Tab 栏全选/取消切换 |
| 主机夹（名称 · 总数 · 当前） | 是（=断开夹内全部） | 同机多连接；悬停二级 Tab / `+`；右键水平/垂直/田字分屏；侧栏 |
| 顶栏「设置」 | — | 快捷键等 |

默认快捷键：`Ctrl+Tab` / `Ctrl+Shift+Tab` **仅在一级会话 Tab 间**循环（不含主机列表）；`Ctrl+Shift+[` / `]` 在并发组窗格或主机夹当前二级 Tab 窗格间切换；`Ctrl+Alt+1`～`4` 聚焦第 N 窗格；`Ctrl+Shift+E` 开关侧栏（工作区 / 远程文件）；`Ctrl+1`～`9` 跳到第 N 个 Tab（1=主机列表）。设置页按连接 / Tab 导航 / 窗格 / 远程文件分组。
...
5. **分屏**：在已连接终端上右键 →「垂直分割」「水平分割」或「田字分割」→ 同机再开连接并排/叠放/四格；拖分割条改比例；关窗格只断该连接。
6. **工作区**：`Ctrl+Shift+E` 开侧栏 →「工作区」→「保存当前」命名；之后可「打开」恢复布局与目录（新开 Tab）。
...
5. **分屏**：终端内右键 → 垂直/水平/田字分割 → 同机新连接出现在**当前二级 Tab**内（不会多出二级 Tab）；`+` 才新建二级 Tab。拖分割条；右键关窗格 / 二级条 × 关整页。分屏默认同组并发（竖线/方块光标）。`Ctrl+Alt+1`～`4` 聚焦窗格。
...
| 8e | 侧栏保存/打开工作区；Ctrl+Alt+1～4 | 命名组合可再开；窗格数字聚焦 |
...
| 20 | 下载该文件到本机 | 系统保存对话框成功，内容正确 |
| 21 | 目录行点「下载」 | 选本机父目录后生成同名文件夹并递归下载；进度显示「n/m 个文件」 |
