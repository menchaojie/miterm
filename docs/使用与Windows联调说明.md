...
默认快捷键：`Ctrl+Tab` / `Ctrl+Shift+Tab` **仅在一级会话 Tab 间**循环（不含主机列表）；`Ctrl+Shift+[` / `]` 在并发组窗格或主机夹当前二级 Tab 窗格间切换；`Ctrl+Alt+←/→/↑/↓` 按布局跳转相邻窗格；`Ctrl+Shift+空格` 交替全部加入/退出并发输入；`Ctrl+Shift+E` 开关侧栏（工作区 / 远程文件）；`Ctrl+1`～`9` 跳到第 N 个 Tab（1=主机列表）。设置页按连接 / Tab 导航 / 窗格 / 远程文件分组。
...
| 8e | 侧栏保存/打开工作区；Ctrl+Alt+方向键 | 命名组合可再开；按布局跳转相邻窗格 |
