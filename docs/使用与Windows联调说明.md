...
| 远程文件 | SSH 会话侧栏「远程文件」打开 SFTP；默认跟随终端目录；单文件/整夹上传与下载；下载完成后可「打开目录」 |
| 工作区 | 侧栏「工作区」保存/打开命名组合（主机夹分屏或并发）；含布局与 cwd；打开时新开一级 Tab；主机列表「收藏会话」按钮开关侧栏 |
...
| 20 | 下载该文件到本机 | 系统保存对话框成功，内容正确；完成后点「打开目录」应在资源管理器中选中该文件 |
| 21 | 目录行点「下载」 | 选本机父目录后生成同名文件夹并递归下载；进度显示「n/m 个文件」；完成后「打开目录」应打开该本地文件夹 |
