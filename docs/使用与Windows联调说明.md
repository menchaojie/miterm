...
| 工作区 | 侧栏「工作区」保存/打开命名组合（主机夹分屏或并发）；可按**独立工作区分类**筛选与归属；含布局与 cwd；打开时新开一级 Tab；主机列表「收藏会话」按钮开关侧栏 |
| 保存命令 | 侧栏「命令」：新建/编辑常用命令；可按**独立命令分类**筛选与归属；点击标题填入当前终端（不回车）；「运行」填入并执行；跟随并发输入同步；Ctrl+Shift+P 命令面板快速搜索；关闭后焦点回到终端 |
...
| 8e2 | 工作区「新建分类」后「保存当前」；列表筛选 / 改分类 / 删工作区分类 | 归属正确；与主机分类互不影响；删工作区分类后该项变未分类 |
...
| 22d | 命令「新建分类」/ 筛选 / 改分类 / 删命令分类 | 与主机、工作区分类互不影响；删命令分类后该项变未分类 |
