...
| 工作区 | 侧栏「工作区」保存/打开命名组合（主机夹分屏或并发）；**按独立工作区分类分组树**展示；含布局与 cwd；打开时新开一级 Tab；主机列表「收藏会话」按钮开关侧栏 |
...
| Tab | 可否关闭 | 内容 |
|-----|----------|------|
| 主机列表（最左，固定） | 否 | 按分类分组树 + 主机表（含本地终端条目）；可勾选多台；「收藏会话」开侧栏 |
| 并发 · N 台 | 是（=断开组内全部） | 同屏网格；浮层「并/单」；Tab 栏全选/取消切换 |
| 主机夹（名称 · 总数 · 当前） | 是（=断开夹内全部） | 同机多连接；悬停二级 Tab / `+`；右键水平/垂直/田字分屏；侧栏 |
| 顶栏「设置」 | — | 快捷键等 |

默认快捷键：`Ctrl+Tab` / `Ctrl+Shift+Tab` **仅在一级会话 Tab 间**循环（不含主机列表）；`Ctrl+Shift+[` / `]` 在并发组窗格或主机夹当前二级 Tab 窗格间切换；`Ctrl+Alt+←/→/↑/↓` 按布局跳转相邻窗格；`Ctrl+Shift+空格` 交替全部加入/退出并发输入；`Ctrl+Shift+E` 开关侧栏（工作区 / 远程文件 / 命令）；`Ctrl+Shift+P` 命令面板；`Ctrl+1`～`9` 跳到第 N 个 Tab（1=主机列表）。设置页按连接 / Tab 导航 / 窗格 / 远程文件分组。
终端内（指针在终端上）默认可用 **Ctrl+滚轮** 缩放字体；设置「连接 → 终端外观」可关闭，或直接改字号。
切换到会话 / 二级 / 窗格后会自动聚焦终端，可直接输入（终端内亦可使用上述快捷键）。

### 数据流简述

1. 「增加 / 编辑」→ 弹窗保存到 SQLite；主机列表按**分类分组树**展示（可折叠）。
2. 「增加主机」可选远程 SSH 或本地终端；本地可归入分类（如「本地」）。点击分类名可设为新建默认归属。
...
6. **工作区**：主机列表点「收藏会话」（或 `Ctrl+Shift+E`）开侧栏 →「工作区」→ 可「新建分类」管理归属（与主机分类独立）→ 会话页内可「保存当前」命名；之后可「打开」恢复布局与目录（新开 Tab）。
7. **命令**：侧栏「命令」可新建常用命令与命令分类（独立于主机/工作区）；`Ctrl+Shift+P` 打开命令面板搜索填入/运行，关闭后面板焦点回到终端。
...
| 8e2 | 工作区「新建分类」后「保存当前」；分组树折叠 / 改分类 / 删工作区分类 | 归属正确；与主机分类互不影响；删工作区分类后该项变未分类 |
