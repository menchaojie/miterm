...
| 主页 | 顶栏「主页」；侧栏常开仅「主机 | 工作区」；主机导航筛选右侧扁平列表（数量右对齐；选中浅底、无左边框条）；工作区点名称开详情弹窗 |
| 工作区 | 侧栏「工作区」保存/打开命名组合；独立分类分组；列表仅名称，详情内打开/删除；打开时新开一级 Tab |
...
| 8e | 主页侧栏「工作区」点名称；会话页 Ctrl+Shift+E；Ctrl+Alt+方向键 | 主页无「命令」Tab；详情可打开工作区；会话页可开关侧栏 |
