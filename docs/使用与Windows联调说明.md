...
| 当前能力 | 密码登录、统一 Tab（主页 + SSH/本地多会话）、本地终端作主机条目、xterm、SQLite 保存主机、无边框圆角窗口（透明） |
| 暂不支持 | 私钥、密码加密存储、目录递归传输等（见 [开发文档 §1](开发文档.md#1-项目概述)） |
| 断线重连 | 设置中可开关自动重连；TCP/SSH keepalive；每会话串行 I/O；`exit` 手动重连，关 Tab 不重连；重连后尽量 `cd` 恢复原目录 |
...
| 主页 | 顶栏「主页」；侧栏常开仅「主机 | 工作区」；主机导航筛选右侧扁平列表（数量右对齐；选中浅底、无左边框条）；工作区单击详情 / 双击打开 |
| 工作区 | 侧栏「工作区」保存/打开命名组合；独立分类分组；列表仅名称；单击详情、双击打开；打开时新开一级 Tab |
| 保存命令 | **仅会话侧栏**「命令」；独立分类；列表仅标题，详情内填入/运行/编辑；Ctrl+Shift+P 命令面板；关闭后焦点回到终端 |
...
│   ├── App.tsx               # 统一 Tab 主页
│   ├── components/
│   │   ├── AppTabBar.tsx     # 主页 + 会话 Tab + 设置 + 窗口控件
│   │   ├── HostListPage.tsx  # 主页右侧主机表（按侧栏筛选）
│   │   ├── HostsNavPanel.tsx # 主页侧栏「主机」分类导航
...
| Tab | 可否关闭 | 内容 |
|-----|----------|------|
| 主页（最左，固定） | 否 | 侧栏常开（主机 \| 工作区）+ 右侧主机表；可勾选多台并发 |
| 并发 · N 台 | 是（=断开组内全部） | 同屏网格；浮层「并/单」；Tab 栏全选/取消切换 |
| 主机夹（名称 · 总数 · 当前） | 是（=断开夹内全部） | 同机多连接；顶部常驻二级条 / `+`；右键水平/垂直/田字分屏；侧栏 |
| 顶栏「设置」 | — | 快捷键等 |

默认快捷键：`Ctrl+Tab` / `Ctrl+Shift+Tab` **仅在一级会话 Tab 间**循环（不含主页）；`Ctrl+Shift+[` / `]` 在并发组窗格或主机夹当前二级 Tab 窗格间切换；`Ctrl+Alt+←/→/↑/↓` 按布局跳转相邻窗格；`Ctrl+Shift+空格` 交替全部加入/退出并发输入；`Ctrl+Shift+E` **会话页**开关侧栏（主页侧栏常开）；`Ctrl+Shift+P` 命令面板；`Ctrl+1`～`9` 跳到第 N 个 Tab（1=主页）。设置页按连接 / Tab 导航 / 窗格 / 远程文件分组。
终端内（指针在终端上）默认可用 **Ctrl+滚轮** 缩放字体；设置「连接 → 终端外观」可关闭，或直接改字号。
切换到会话 / 二级 / 窗格后会自动聚焦终端，可直接输入（终端内亦可使用上述快捷键）。

### 数据流简述

1. 「增加 / 编辑」→ 侧栏「主机」点「增加主机」或行内编辑；弹窗保存到 SQLite。
2. 「增加主机」可选远程 SSH 或本地终端；本地可归入分类（如「本地」）。侧栏点分类可设为新建默认归属。
3. 点名称连接 → 若该主机夹已存在则只激活；否则新建主机夹 + 首条会话（`ssh_connect` 或 `local_connect`）。
4. 主机夹内点顶部细条 `+` → 同机再开一条连接；关二级只断一条。
5. **分屏**：在已连接终端上右键 →「垂直分割」「水平分割」或「田字分割」→ 同机再开连接并排/叠放/四格；拖分割条改比例；关窗格只断该连接。右键「复制」（有选区）/「粘贴」（写入当前窗格，跟随并发同步）。
6. **工作区**：主页侧栏切到「工作区」（或会话页 `Ctrl+Shift+E`）→ 可「新建分类」管理归属 → 会话页内可「保存当前」命名；之后可「打开」恢复布局与目录（新开 Tab）。
7. **命令**：侧栏「命令」可新建常用命令与命令分类；`Ctrl+Shift+P` 打开命令面板搜索填入/运行，关闭后面板焦点回到终端。
8. 本地条目右侧**设置**可改 Shell / 路径（如 Git Bash）。
9. 关一级夹 / 对端关闭 → 断开对应会话。
...
1. 启动后默认在 **主页** Tab（侧栏常开）。
2. 侧栏「主机」选全部或某分类；可新建/重命名/删除分类；右侧列表同步过滤。
3. 点 **增加主机**（侧栏）保存信息；点 **名称** 登录，右侧出现主机夹 Tab。再次点同一名称只切回该夹，不新建连接。
4. **同机多连**：终端顶部常驻二级条 → 点 **+** 再开一条；显示变为如叠层 `2` · 终端 `2`（先总数、后当前）。
...
7. 顶栏在「主页」与各一级 Tab 之间切换（也可用快捷键）；一级 **×** 断开夹内全部；二级 **×** 只断一条。
...
| 5c | 顶部细条 `+` 再连 → 关二级 × | 一级标题数量增减正确；最后一条二级关掉后夹消失 |
| 6 | 增加主机 → 本地终端（如 Git Bash + 路径） | 列表出现普通条目（可归类），无固定首行 |
| 7 | 点本地条目名称 / 右侧设置图标 | 打开主机夹可交互；设置可改 Shell 与路径 |
| 8 | 终端内 Ctrl+Tab / Ctrl+1～9 | Ctrl+Tab 只在一级 Tab 间轮换（含并发组与主机夹）；Ctrl+1 回主页 |
...
| 8e | 主页侧栏「工作区」单击名称 / 双击名称；会话页 Ctrl+Shift+E；Ctrl+Alt+方向键 | 单击开详情；双击直接打开新 Tab；会话页可开关侧栏 |
| 8e2 | 侧栏「主机」选分类 / 新建分类；右侧过滤；删主机分类 | 右侧同步过滤；数量列右对齐；选中为浅底无左边框条；与工作区/命令分类互不影响 |
| 8e3 | 会话侧栏「命令」点标题 | 弹窗显示正文；可填入/运行/编辑/删除 |
...
| 11 | Tab 栏并发图标按钮 | 全选↔全部取消同一位置切换图标；每格顶部常驻细条可操作 |
| 12 | 主机夹二级条 | 常驻细条（约 20px，色深于一级顶栏）；不遮挡终端正文 |
| 13 | 远程 `exit` | 终端提示已退出；出现「重新连接」「关闭窗口」；不自动重连 |
| 14 | 模拟异常断线（或关服务） | 开启自动重连时打印重连文案并恢复；关 Tab 不会重连 |
| 14b | SSH 会话连续输入 / Tab 补全一段时间 | 不应无故「正在重连」；若仍出现请查看 stderr 中 `[ssh] channel read error` |
...
| 22 | 会话侧栏「命令」→ 新建 | 弹窗保存标题与命令内容后出现在列表；可选分类 |
| 22b | 点击命令标题 | 打开详情弹窗；可填入（不回车）或运行 |
| 22c | 详情「运行」 | 填入并回车执行；若该窗格在并发同步中应同步到已加入会话 |
| 22d | 命令「新建分类」/ 分组树 / 改分类 / 删命令分类 | 与主机、工作区分类互不影响；删命令分类后该项变未分类 |
| 22e | Ctrl+Shift+P 命令面板 | 搜索命令；Enter 填入、Shift+Enter 运行；无连接时提示不可用；关闭后可直接在终端按 Enter（无需再点终端） |
| 22f | 主页侧栏 | 仅「主机」「工作区」；无「命令」「远程文件」 |
