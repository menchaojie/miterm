...
默认快捷键：`Ctrl+Tab` / `Ctrl+Shift+Tab` **仅在一级会话 Tab 间**循环（不含主机列表）；`Ctrl+Shift+[` / `]` 在并发组窗格或主机夹二级会话间切换；`Ctrl+Shift+E` 开关远程文件侧栏；`Ctrl+1`～`9` 跳到第 N 个 Tab（1=主机列表）。设置页按连接 / Tab 导航 / 窗格 / 远程文件分组。
