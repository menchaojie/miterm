...
| 主页 | 顶栏「主页」；侧栏常开「主机」导航（全部/分类/未分类）筛选右侧扁平列表；增主机与分类管理在侧栏；另有工作区 / 命令 |
| 工作区 | 侧栏「工作区」保存/打开命名组合（主机夹分屏或并发）；**按独立工作区分类分组树**展示；含布局与 cwd；打开时新开一级 Tab |
| 保存命令 | 侧栏「命令」：新建/编辑常用命令；**按独立命令分类分组树**展示；点击标题填入当前终端（不回车）；「运行」填入并执行；跟随并发输入同步；Ctrl+Shift+P 命令面板快速搜索；关闭后焦点回到终端 |
...
| Tab | 可否关闭 | 内容 |
|-----|----------|------|
| 主页（最左，固定） | 否 | 侧栏常开（主机导航 + 工作区/命令）+ 右侧主机表；可勾选多台并发 |
...
| 8e | 主页侧栏「工作区」；会话页 Ctrl+Shift+E；Ctrl+Alt+方向键 | 主页侧栏常开；会话页可开关；命名组合可再开；按布局跳转相邻窗格 |
