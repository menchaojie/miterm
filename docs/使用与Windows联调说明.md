...
| 主机夹（名称 · 总数 · 当前） | 是（=断开夹内全部） | 同机多连接；悬停二级 Tab / `+`；右键水平/垂直分屏 |
| 顶栏「设置」 | — | 快捷键等 |
...
5. **分屏**：在已连接终端上右键 →「垂直分割（左右）」或「水平分割（上下）」→ 再开一条同机连接并排/叠放显示；拖分割条改比例；可再右键继续拆；关窗格只断该连接。
...
5. **分屏**：终端内右键 → 垂直/水平分割 → 同机新连接分窗显示（仅分割线，无窗格标题）；拖分割条；二级条 **×** 或右键「关闭此窗格」只断该格。分屏后默认并发输入（竖线光标=已加入，方块=未加入）；右键可加入/退出；一级 Tab 四格图标全选/取消。
...
| 8c | 主机夹终端右键 → 垂直/水平分割 | 同机新连接分屏；可拖分割条；关窗格只断该格 |
