...
| 远程文件 | SSH 会话点「文件」打开 SFTP 侧栏；默认跟随终端目录；单文件/整夹上传与下载 |
...
| 主机夹（名称 · 总数 · 当前） | 是（=断开夹内全部） | 同机多连接；悬停顶部显示二级 Tab 与 `+` |
| 顶栏「设置」 | — | 快捷键等 |

默认快捷键：`Ctrl+Tab` / `Ctrl+Shift+Tab` **仅在一级会话 Tab 间**循环（不含主机列表）；`Ctrl+Shift+[` / `]` 在并发组窗格或主机夹二级会话间切换；`Ctrl+1`～`9` 跳到第 N 个 Tab（1=主机列表）。
...
5. 本地条目右侧**设置**可改 Shell / 路径（如 Git Bash）。
6. 关一级夹 / 对端关闭 → 断开对应会话。
...
5. **本地终端**：「增加主机」选本地终端 → 选 Shell（Git Bash / 自定义可填路径）→ 保存到列表（建议分类「本地」）→ 点名称打开。
6. 顶栏在「主机列表」与各一级 Tab 之间切换（也可用快捷键）；一级 **×** 断开夹内全部；二级 **×** 只断一条。
7. **并发会话**：勾选至少 2 台主机 → 点「并发会话」同时连接；网格同屏。悬停格子时浮层可点「并/单」；Tab 栏图标在「全选 / 全部取消」间切换。关组 Tab 断开全部。
8. 点顶栏 **设置** 可修改快捷键；本地终端路径在对应条目的设置图标中修改。
...
| 19 | 「上传文件…」选小文件 | 列表中出现该文件 |
| 19b | 「上传文件夹…」选本机目录 | 远端出现同名文件夹及内容；进度显示「n/m 个文件」 |
