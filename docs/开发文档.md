...
- `sftp_list` / `sftp_upload` / `sftp_download` / `sftp_download_dir`（SSH 会话；独立 SFTP 旁路连接）
- SQLite 保存主机（含 `conn_type` / `shell` / `shell_path`）与分类
...
│   ├── settings.ts            # 快捷键 + 本地 Shell 多选、localStorage
...
- **设置**（`localStorage` `miterm.settings.v1`）：Tab / 窗格快捷键、断线自动重连开关与最大次数等。
...
- **SFTP 远程文件**：SSH 会话连接后，主机夹二级条或并发格浮层点「文件」打开**左侧**侧栏；**树形展开**浏览目录（懒加载）；**默认跟随**终端 `cwd`（`cd` 会变，敲普通命令不误跳 `~/`：忽略无效 cwd，且提示符误报的「非家→家」跳转会被丢掉）；路径栏「⌖」转到终端目录，「⇄」可关闭跟随以便自由浏览；可上传/下载；进度经 `sftp-progress`；SFTP 旁路连接，不堵 PTY。
...
- 自定义命令：`ssh_connect`、`local_connect`、`ssh_write`、`ssh_resize`、`ssh_disconnect`、`sftp_list`、`sftp_upload`、`sftp_download`、`sftp_download_dir`
...
- **主机夹**：`TerminalWorkspace` 叠放同机会话层；顶部热区悬停显示二级 Tab 与 `+`
...
2. 断线自动重连 — **已做**
3. 多主机同屏（并发会话网格格子）— **已做**
4. SFTP（Rust `sftp`）— **已做**（`sftp_list` / `sftp_upload` / `sftp_download` / `sftp_download_dir`；侧栏面板）
