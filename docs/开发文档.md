...
- **SFTP 远程文件**：SSH 会话连接后，主机夹二级条或并发格浮层点「文件」打开**左侧**侧栏；**树形展开**浏览目录（懒加载）；**默认跟随**终端 `cwd`；支持单文件/整夹上传与下载（整夹跳过符号链接，单次最多 8000 文件）；进度经 `sftp-progress`；下载完成后进度条旁「打开目录」调用 `reveal_path`（Windows 选中文件 / 打开文件夹）；SFTP 旁路连接，不堵 PTY。
...
| 2026-08-10 | **SFTP 下载打开目录**：`reveal_path`；下载完成后进度条旁「打开目录」在资源管理器定位文件或打开文件夹。 |
