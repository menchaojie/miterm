...
- **设置**（`localStorage` `miterm.settings.v1`）：Tab / 窗格快捷键（含 Ctrl+Alt+1～4）、断线自动重连、Ctrl+滚轮缩放终端字体开关、终端字号等。
- **工作区**（SQLite `workspaces`）：命名保存主机夹/并发组合（布局树 + cwd）；侧栏打开；不存瞬时 sessionId。
- **侧栏**：工作区 | 远程文件；`Ctrl+Shift+E` 开关。
...
| 2026-08-04 | **工作区侧栏**：SQLite 保存/打开命名组合；侧栏 Tab「工作区 / 远程文件」；打开总是新开一级 Tab；Ctrl+Alt+1～4 聚焦窗格。 |
