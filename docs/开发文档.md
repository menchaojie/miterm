...
- 主页侧栏「主机」：全部 / 各分类 / 未分类导航；新建/重命名/删除分类；选中项驱动右侧过滤（扁平列表，不再二次分组）。
...
- **工作区**（SQLite `workspaces`）：命名保存主机夹/并发组合（布局树 + cwd）；可选 `category_id` → **`workspace_categories`**（与主机分类独立）；侧栏**分组树**展示；可新建/重命名/删除工作区分类；「保存到」选择归属；打开；不存瞬时 sessionId。
- **保存命令**（SQLite `saved_commands`）：侧栏「命令」Tab；CRUD；可选 `category_id` → **`command_categories`**（与主机/工作区分类独立）；侧栏**分组树**展示；新建/重命名/删除命令分类；编辑时可选归属；点击标题填入当前聚焦终端（不回车）；「运行」末尾补换行执行；写入走与键盘相同的并发同步规则。
- **命令面板**：默认 `Ctrl+Shift+P`（可改）；搜索收藏命令；Enter 填入、Shift+Enter 运行；跟随并发同步；关闭后通过 `focusSessionTerminal` 把焦点还给当前 xterm。
- **侧栏**：主页常开（主机 | 工作区 | 命令，无关闭）；会话页（工作区 | 远程文件 | 命令）可用 `Ctrl+Shift+E` 开关。
