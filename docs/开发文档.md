...
- **SFTP 远程文件**：SSH 会话连接后，主机夹二级条或并发格浮层点「文件」打开**左侧**侧栏；**树形展开**浏览目录（懒加载）；默认进入当前终端 `cwd`；可上传单文件、下载单文件或**整夹递归下载**；传输进度经 `sftp-progress`（含 `fileIndex`/`fileCount`）；SFTP 使用**独立 SSH 连接**，与终端 PTY 分离。
