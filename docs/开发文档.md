...
- **工作区**（SQLite `workspaces`）：命名保存主机夹/并发组合（布局树 + cwd）；可选 `category_id` → **`workspace_categories`**（与主机分类独立）；侧栏分组；列表仅名称，点击弹窗详情（打开/改分类/删除）。
...
- **断线与重连**：TCP + SSH keepalive 降低空闲掉线；异常断线时终端打印「连接已断开 → 正在重连…」，成功后继续显示远程 banner；`exit` 等干净退出不自动重连，底部可点「重新连接」或「关闭窗口」；关 Tab / × 不重连。
