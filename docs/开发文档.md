...
- **主机夹分屏**：在已连接会话上**右键** →「垂直分割（左右）」/「水平分割（上下）」→ 同机新建连接并拆窗格；可继续在窗格上再分；仅一条分割线分隔（可拖改比例）；右键或顶部二级条可关该格；布局为二叉树（`HostFolder.layout` / `src/splitLayout.ts`）；未分屏时仍为二级 Tab 叠层。
- **分屏并发输入**：分屏后各窗格默认加入并发（键入同步）；光标规则与并发组相同——已加入/焦点为竖线，未加入为实心块；右键可加入/退出；一级 Tab 上四格图标可全选/全部取消。
...
- **SFTP 远程文件**：SSH 会话连接后，主机夹二级条或并发格浮层点「文件」打开**左侧**侧栏；**树形展开**浏览目录（懒加载）；**默认跟随**终端 `cwd`；支持单文件/整夹上传与下载（整夹跳过符号链接，单次最多 8000 文件）；进度经 `sftp-progress`；SFTP 旁路连接，不堵 PTY。
