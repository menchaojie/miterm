...
- **工作区**（SQLite `workspaces`）：命名保存主机夹/并发组合（布局树 + cwd）；可选 `category_id` → **`workspace_categories`**（与主机分类独立）；侧栏可新建/删除工作区分类、筛选与「保存到」；打开；不存瞬时 sessionId。
- **保存命令**（SQLite `saved_commands`）：侧栏「命令」Tab；CRUD；可选 `category_id` → **`command_categories`**（与主机/工作区分类独立）；侧栏可新建/删除命令分类；筛选与编辑时归属；点击标题填入当前聚焦终端（不回车）；「运行」末尾补换行执行；写入走与键盘相同的并发同步规则。
