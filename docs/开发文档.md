...
- 主页：登录 / 编辑 / 删除；侧栏「主机」导航筛选；右侧按「全部 / 分类 / 未分类」展示
...
- SQLite 保存主机（含 `conn_type` / `shell` / `shell_path`）与主机分类、工作区（含**独立** `workspace_categories`）、**常用命令**（`saved_commands` + **独立** `command_categories`）
- **主页 / 终端多 Tab 页**（可回主页而不断开）
...
│   │   ├── HostsNavPanel.tsx  # 主页侧栏「主机」分类导航 / 管理
...
│   └── hooks/
│       └── useSshTerminal.ts  # 按 sessionId 绑定 IPC / 事件 / focusSessionTerminal
└── src-tauri/
    ├── Cargo.toml
    ├── capabilities/
    │   └── default.json
...
| 区域 | 说明 |
|------|------|
| **顶栏 Tab** | 最左固定「主页」（图标+文字，不可关闭）；右侧为「并发 · N 台」组 Tab 与**主机夹** Tab；「设置」打开配置 |
| **内容区** | 选中「主页」：左侧常开侧栏 + 右侧主机表；选中主机夹/并发组显示对应工作区 |

说明：

- 「增加主机」在主页侧栏「主机」Tab；可选 **远程主机（SSH）** 或 **本地终端**；本地可放进分类（如「本地」）。
- 主页侧栏「主机」：全部 / 各分类 / 未分类导航；新建/重命名/删除分类；选中项驱动右侧过滤（扁平列表，不再二次分组）。选中样式与右侧主机行一致（浅底高亮、无左边框条）；主机数量右对齐、各行数字列对齐。
- **点名称连接**：若该 `savedHostId` 的主机夹已存在 → **只激活该夹、不新建连接**；否则新建主机夹 + 第一条会话（`ssh_connect` / `local_connect`）。
- **主机夹**：一级 Tab 为「名称 · 叠层图标+总数 · 终端图标+当前序号」；工作区顶部**常驻细条**显示二级 Tab（图标+序号）与 **+**（同机再开一条连接）；关二级 = 断一条；关一级 = 断夹内全部；最后一条二级关掉后夹消失。
...
- **终端右键**：有选区可「复制」；已连接时可「粘贴」（读系统剪贴板写入当前窗格，跟随并发同步）；主机夹另有分屏项。
- 本地条目右侧为**设置**图标（编辑 Shell 与路径）；远程为编辑图标。
- 切回「主页」时终端保持挂载（隐藏），会话继续收输出。
- **设置**（`localStorage` `miterm.settings.v1`）：Tab / 窗格快捷键（含 Ctrl+Alt+方向键）、断线自动重连、Ctrl+滚轮缩放终端字体开关、终端字号等。
- **工作区**（SQLite `workspaces`）：命名保存主机夹/并发组合（布局树 + cwd）；可选 `category_id` → **`workspace_categories`**（与主机分类独立）；侧栏分组；列表仅名称，**单击**弹窗详情（打开/改分类/删除），**双击**直接打开。
- **保存命令**（SQLite `saved_commands`）：**仅会话侧栏**「命令」Tab；可选 `category_id` → **`command_categories`**；列表仅标题，点击弹窗（填入/运行/编辑/删除）。
- **命令面板**：默认 `Ctrl+Shift+P`（可改）；搜索收藏命令；Enter 填入、Shift+Enter 运行；跟随并发同步；关闭后通过 `focusSessionTerminal` 把焦点还给当前 xterm。
- **侧栏**：主页常开且仅「主机 | 工作区」；会话页「工作区 | 远程文件 | 命令」可用 `Ctrl+Shift+E` 开关。
- **断线与重连**：TCP + SSH keepalive 降低空闲掉线；异常断线时终端打印「连接已断开 → 正在重连…」，成功后继续显示远程 banner；`exit` 等干净退出不自动重连，底部可点「重新连接」或「关闭窗口」；关 Tab / × 不重连。每个 SSH 会话由**独立 I/O 线程串行**处理 read/write/resize/keepalive（短超时阻塞），避免非阻塞 EAGAIN 交叉调用导致输入时误断。
...
- **关 Tab**：先移除 UI，再向该会话 I/O 线程发 `Stop` 并 join；多会话彼此独立并行。
- **SFTP 远程文件**：SSH 会话连接后，主机夹二级条「侧栏」或快捷键打开**左侧**侧栏（并发组用侧栏快捷键 / 一级操作，格内不再放「文件」按钮）；**树形展开**浏览目录（懒加载）；**默认跟随**终端 `cwd`；支持单文件/整夹上传与下载（整夹跳过符号链接，单次最多 8000 文件）；进度经 `sftp-progress`（完成后收起）；下载成功后该行「下载」左侧显示「已下载」，点击调用 `reveal_path` 打开本机位置；本机路径不存在时自动清除标记并友好提示；悬停「已下载」可点 × 主动清除标记（不删本机文件）；SFTP 旁路连接，不堵 PTY。
- Ctrl+Tab / Ctrl+Shift+Tab：**仅一级会话 Tab 间**循环（含并发组与主机夹）；回主页用 Ctrl+1 或点击。
- Ctrl+Shift+[ / ]：并发组内切换窗格，或主机夹内切换二级会话（可设置中改键）。
- Ctrl+Shift+E：会话页开关侧栏；主页侧栏常开（不关闭）。
- Ctrl+Shift+P：命令面板（搜索收藏命令并填入/运行；可设置中改键）。
- 切到会话 Tab / 二级会话后自动聚焦 xterm，可直接键入。
- **并发会话**：主页勾选 ≥2 台 →「并发会话」同时连接；同一组 Tab 内以接近方形的网格同屏；每格顶部常驻细条可「并/单」加入并发输入；Tab 栏同一位置按钮交替全选/全部取消；键入同步到已加入会话；参与格光标为竖线、未参与为方块（不闪烁）；关组 Tab 断开组内全部。
...
1. `TcpStream::connect((host, port))` + TCP keepalive
...
5. 启动**每会话一条 I/O 线程**（会话间并行；线程内串行）：
   - 阻塞模式 + `set_timeout(50ms)`，循环：处理写/resize 命令队列 → 周期性 `keepalive_send`（`want_reply=false`）→ `channel.read`
   - 输出：`emit("ssh-output", { sessionId, data })`
6. 对端关闭 / 读错误 → `emit("ssh-closed", { sessionId, reason: "remote"|"error" })`；主动关 Tab 发 `Stop`，不误触发重连

> 说明：旧实现用非阻塞读且在 EAGAIN 后释放锁，写/keepalive 可插队，易触发 libssh2 状态损坏并在**输入时**异常断线；现已改为单线程串行 I/O。
...
表 `categories`：主机分类（`id`, `name`, `sort_order`）。  
表 `workspace_categories` / `command_categories`：工作区与命令各自独立的分类表（结构同主机分类）；名称可与主机分类相同，**id 不共用**。启动迁移 `migrate_domain_categories`（`schema_meta.domain_categories_v1`）会按名称把旧的共用分类引用拆到新表。  
...
| `list_categories` / `create_category` / `update_category` / `delete_category` | **主机**分类 CRUD；删分类后主机变未分类（不影响工作区/命令） |
| `list_workspace_categories` / `create_workspace_category` / `update_workspace_category` / `delete_workspace_category` | **工作区**分类 CRUD；删后工作区变未分类 |
| `list_command_categories` / `create_command_category` / `update_command_category` / `delete_command_category` | **命令**分类 CRUD；删后命令变未分类 |

表 `workspaces`：命名工作区（`kind` + JSON `payload` + 可选 `category_id` → `workspace_categories`）。  
表 `saved_commands`：`id` / `title` / `body` / `sort_order` / `updated_at` / 可选 `category_id` → `command_categories`。

| 命令 | 参数 | 说明 |
|------|------|------|
| `list_workspaces` / `save_workspace` / `update_workspace` / `delete_workspace` | 工作区 CRUD；save/update 可带 `categoryId`（工作区分类） | 见侧栏「工作区」 |
| `list_commands` | — | 常用命令列表（按 `sort_order`） |
| `save_command` | `{ params: { title, body, categoryId? } }` | 新增（`categoryId` 为命令分类） |
| `update_command` | 同上 + `id` | 编辑（含改分类） |
...
- **主机夹**：`TerminalWorkspace` 叠放同机会话层；顶部常驻细条显示二级 Tab 与 `+`；右键分屏见 `splitLayout.ts`
...
| 2026-08-10 | **终端右键粘贴**：主机夹/并发组菜单「粘贴」；读剪贴板经并发同步写入。 |
| 2026-08-13 | **工作区分类**：`workspaces.category_id` 初版共用主机分类；侧栏筛选 / 保存到 / 逐条改分类。 |
| 2026-08-13 | **命令分类**：`saved_commands.category_id` 初版共用同一套分类；侧栏筛选 / 新建编辑归属 / 逐条改分类。 |
| 2026-08-15 | **命令面板**：默认 Ctrl+Shift+P；搜索收藏命令；Enter 填入 / Shift+Enter 运行；跟随并发同步。 |
| 2026-08-15 | **分类拆分**：`workspace_categories` / `command_categories` 与主机 `categories` 独立；按名迁移旧引用；侧栏各自新建/删除分类。 |
| 2026-08-15 | **命令面板焦点**：关闭面板后 `focusSessionTerminal` 恢复当前 xterm，无需再点终端即可输入。 |
| 2026-08-15 | **分类分组树**：主机 / 工作区 / 命令改为可折叠分组列表（非筛选下拉）；折叠状态记 localStorage。 |
| 2026-08-15 | **主页侧栏主机导航**：顶栏「主页」图标+文字；主页侧栏常开（主机\|工作区\|命令）；右侧按分类过滤；去掉「收藏会话」。 |
| 2026-08-16 | **侧栏精简**：主页仅主机\|工作区；命令仅会话页；工作区/命令列表只显示名称，点击弹窗详情。 |
| 2026-08-16 | **UI 清新化**：克制色板与圆角；主按钮浅底细边；弱化远程文件/弹层高亮。 |
| 2026-08-16 | **主机分类导航**：数量右对齐对齐；选中改为与主机行一致的浅底高亮（去掉左边框条）。 |
| 2026-08-16 | **顶栏推开正文**：主机夹二级条 / 并发格浮层悬停展开时下推终端，避免遮挡顶部输出；离开延迟收起。 |
| 2026-08-16 | **顶栏常驻细条**：改回固定占用约 24–26px；主机夹二级条与并发格顶栏始终可见，不再悬停遮挡或抖动。 |
| 2026-08-16 | **两级 Tab 收细分层**：一级约 22px（偏亮灰蓝），二级约 20px（更深并带冷色描边），层次更清晰。 |
| 2026-08-16 | **并发格去掉「文件」**；会话 `exit`/出错底部增加「关闭窗口」（与「重新连接」并列）。 |
| 2026-09-05 | **工作区双击打开**：侧栏单击仍开详情；双击直接打开（延迟单击避免弹窗闪烁）。 |
| 2026-09-05 | **SSH 串行 I/O**：每会话独立线程串行 read/write/resize/keepalive；去掉非阻塞 EAGAIN 交叉调用，修复输入时异常断线。 |
