...
- **工作区**（SQLite `workspaces`）：命名保存主机夹/并发组合（布局树 + cwd）；可选 `category_id`（与主机共用 `categories`）；侧栏筛选与「保存到」；打开；不存瞬时 sessionId。
- **保存命令**（SQLite `saved_commands`）：侧栏「命令」Tab；CRUD；可选 `category_id`（与主机/工作区共用分类）；筛选与编辑时归属；点击标题填入当前聚焦终端（不回车）；「运行」末尾补换行执行；写入走与键盘相同的并发同步规则。
...
表 `workspaces`：命名工作区（`kind` + JSON `payload` + 可选 `category_id`，与主机共用分类）。  
表 `saved_commands`：`id` / `title` / `body` / `sort_order` / `updated_at` / 可选 `category_id`。

| 命令 | 参数 | 说明 |
|------|------|------|
| `list_workspaces` / `save_workspace` / `update_workspace` / `delete_workspace` | 工作区 CRUD；save/update 可带 `categoryId` | 见侧栏「工作区」 |
| `list_commands` | — | 常用命令列表（按 `sort_order`） |
| `save_command` | `{ params: { title, body, categoryId? } }` | 新增 |
...
| 2026-08-13 | **工作区分类**：`workspaces.category_id` 共用主机分类；侧栏筛选 / 保存到 / 逐条改分类。 |
| 2026-08-13 | **命令分类**：`saved_commands.category_id` 共用同一套分类；侧栏筛选 / 新建编辑归属 / 逐条改分类。 |
