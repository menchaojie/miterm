...
- **工作区**：侧栏可保存/打开命名的主机夹或并发组合（含分屏布局与各窗 cwd）；可按与主机共用的分类筛选/归属；总是新开一级 Tab；主机列表右上角「收藏会话」可开关侧栏（默认工作区 Tab）
- **保存命令**：侧栏「命令」Tab 可新建/编辑常用命令；可按与主机共用的分类筛选/归属；点击标题填入当前终端（不回车）；「运行」填入并回车；跟随并发输入同步规则
- **终端字号**：设置可改字号；默认启用 Ctrl+滚轮在终端上缩放字体（仅终端，不影响界面）；可关闭该手势
- 设置中可自定义快捷键（默认 Ctrl+Tab 轮换会话 Tab；Ctrl+Shift+[ / ] 切窗格；Ctrl+Alt+方向键跳转相邻窗格；Ctrl+Shift+空格交替全部加入/退出并发；Ctrl+Shift+E 开关侧栏；Ctrl+1～9；终端内可用）
