...
- 设置中可自定义快捷键（默认 Ctrl+Tab 轮换会话 Tab；Ctrl+Shift+[ / ] 切并发窗格；Ctrl+Shift+E 开关远程文件；Ctrl+1～9；终端内可用）
- **终端字号**：设置可改字号；默认启用 Ctrl+滚轮在终端上缩放字体（仅终端，不影响界面）；可关闭该手势
- 切换到会话 Tab 后自动聚焦终端，可直接输入
- **并发会话**：勾选多台主机一键同屏连接（网格格子）；格内键入可同步到已加入「并发输入」的会话
- **主机夹分屏**：右键垂直/水平分割在**当前二级 Tab**内加窗格（不自动新建二级 Tab）；`+` 才新建二级 Tab；可拖分割条；分屏支持并发输入（竖线/方块光标）；右键可加入/退出/全部加入/全部退出并发；选中文本可复制；Ctrl+点击打开链接
- SQLite 保存主机（明文密码，本机便利）
- xterm 终端 + 窗口 resize 同步 PTY
- **SFTP**：SSH 会话「文件」侧栏浏览远程目录；上传支持菜单选文件/夹，或拖放到上传区确认后上传；支持整夹下载
