...
- **本地终端**：作为主机条目（与 SSH 同级，可分类）；增加主机时选择类型；Git Bash 等可配置路径；主机列表按分类**分组树**展示
- **主机夹分屏**：右键垂直/水平/**田字**分割在**当前二级 Tab**内加窗格（不自动新建二级 Tab）；`+` 才新建二级 Tab；可拖分割条；分屏支持并发输入；右键可加入/退出/全部加入/全部退出并发；选中文本可复制；右键可粘贴；Ctrl+点击打开链接
- **工作区**：侧栏可保存/打开命名的主机夹或并发组合（含分屏布局与各窗 cwd）；按**独立工作区分类分组树**展示；总是新开一级 Tab；主机列表右上角「收藏会话」可开关侧栏（默认工作区 Tab）
...
- 设置中可自定义快捷键（默认 Ctrl+Tab 轮换会话 Tab；Ctrl+Shift+[ / ] 切窗格；Ctrl+Alt+方向键跳转相邻窗格；Ctrl+Shift+空格交替全部加入/退出并发；Ctrl+Shift+E 开关侧栏；Ctrl+Shift+P 命令面板；Ctrl+1～9；终端内可用）
