...
- **工作区**：侧栏可保存/打开命名的主机夹或并发组合（含分屏布局与各窗 cwd）；可按**独立工作区分类**筛选/归属（与主机分类无关）；总是新开一级 Tab；主机列表右上角「收藏会话」可开关侧栏（默认工作区 Tab）
- **保存命令**：侧栏「命令」Tab 可新建/编辑常用命令；可按**独立命令分类**筛选/归属；点击标题填入当前终端（不回车）；「运行」填入并回车；跟随并发输入同步规则；**Ctrl+Shift+P** 打开命令面板快速搜索填入/运行；关闭面板后焦点回到当前终端
